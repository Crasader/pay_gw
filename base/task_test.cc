...
TEST_F(TaskDeathTest, DestroyIncompleteTask) {
  ...
  EXPECT_DEATH(Task(DoNothing, &executor),
...
TYPED_TEST(TaskTest, Return) {
  TypeParam s;
  Status status(error::INVALID_ARGUMENT, "expected status");
...
TYPED_TEST(TaskTest, MultipleReturn) {
  TypeParam s;
  Status status1(error::INVALID_ARGUMENT, "expected status");
  Status status2(error::DEADLINE_EXCEEDED, "unexpected status");
...
TYPED_TEST(TaskTest, ChildBlocksDone) {
  ...
  Task* const child_task(s.task()->AddChild(DoNothing));
  EXPECT_FALSE(s.task()->CancelRequested());
  EXPECT_TRUE(s.task()->IsActive());
  EXPECT_FALSE(s.task()->IsDone());
