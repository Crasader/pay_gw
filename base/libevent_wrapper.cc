...
void DelayCancel(event* timer, mpr_pgw::Task* task) {
  event_del(timer);
  task->Return(mpr_pgw::Status::CANCELLED);
}


void DelayDispatch(evutil_socket_t, short, void* userdata) {
  static_cast<mpr_pgw::Task*>(CHECK_NOTNULL(userdata))->Return();
}
...
void Base::Delay(const duration<double>& delay, Task* task) {
