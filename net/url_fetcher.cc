...
using mpr_pgw::internal::ConnectionPool;
using std::bind;
using std::endl;
using std::make_pair;
using std::move;
...
htp_method VerbToCmdType(UrlFetcher::Verb verb) {
  switch (verb) {
    case UrlFetcher::Verb::GET:     return htp_method_GET;
    case UrlFetcher::Verb::POST:    return htp_method_POST;
    case UrlFetcher::Verb::PUT:     return htp_method_PUT;
    case UrlFetcher::Verb::DELETE:  return htp_method_DELETE;
  }
...
State::State(libevent::Base* base, ConnectionPool* pool,
  ...
  if (request_.url.Protocol() != "http" &&
    ...
    task_->Return(Status(error::INVALID_ARGUMENT,
...
void State::RunRequest() {
  ...
  if (!conn_->connection() || conn_->GetErrored()) {
    conn_.reset();
    task_->Return(Status(error::UNAVAILABLE, "connection failed."));
    return;
  }
  ...
  if (evhtp_make_request(conn_->connection(), http_req, verb,
    ...
    task_->Return(Status(error::INTERNAL, "evhtp_make_request error"));
    return;
  }
  ...
  if (!request_.body.empty()) {
    if (evbuffer_add_reference(bufferevent_get_output(
      ...
      task_->Return(
          Status(error::INTERNAL, "could not set the request body"));
      return;
    }
  }
}
...
void State::RequestDone(evhtp_request_t* req) {
  ...
  if (response_->status_code < 100) {
    ...
    switch (response_->status_code) {
      case kTimeout:
        status =
            Status(error::DEADLINE_EXCEEDED, "connection timed out");
        break;
      case kSSLErrorStatus:
        // There was a problem communicating with the remote host.
        status = Status(error::UNAVAILABLE, "SSL connection failed");
        break;
      case kUnknownErrorStatus:
        status = Status(error::UNAVAILABLE, "connection failed");
        break;
      default:
        LOG(WARNING) << "Unknown status code encountered: "
                     << response_->status_code;
        status =
            Status(error::UNKNOWN, "unknown status code encountered");
    }
    task_->Return(status);
    return;
  }
